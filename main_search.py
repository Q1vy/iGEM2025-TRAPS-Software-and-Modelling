...
def query_sc(query:pd.Series, reference_dataset:pd.DataFrame)->pd.DataFrame:
    ...
    nts = set(reference_dataset.SEQUENCE.sample(100, replace=True).sum()) | set(reference_dataset.SEQUENCE.iloc[:10].sum()) | set(reference_dataset.SEQUENCE.iloc[-10:].sum()) | set(query.SEQUENCE)
    if len(nts) != 4: 
...
def sc_summarize(query:pd.Series, references_scr:pd.DataFrame, target_scr:pd.Series, use_weights = True, VIR_max = 10, r2t = True, plot = False) -> pd.Series:
    ''' function summarizing the result table from query_sc
        ...
        use_weights     whether to use WEIGHT column
        VIR_max         number of very important references per metric saved in the results 
        r2t             normalize references results relative to target results
    ...
    if use_weights:  # check if the input data is okay to use weighted evalutation
        if "WEIGHT" in references_scr.columns:
            ...
            if invalid.any():  warnings.warn(f"{invalid.sum()} entries with nan or inf WEIGHT in references_scr. The script might not behave as intended. Consider setting use_weights = False.")
    ...
    if r2t:
        ...
        if use_weights:  # check if the input data is okay to use weighted evalutation
            if "WEIGHT" in target_scr:
                if np.isinf(target_scr.WEIGHT) or  np.isnan(target_scr.WEIGHT): warnings.warn(f"target_scr has nan or inf WEIGHT. The script might not behave as intended. Consider setting use_weights = False.")
...
def query_sc_summary(query:pd.Series, reference_dataset:pd.DataFrame, target:pd.Series = None, use_weights: bool = True, VIR_max: int = 10, r2t: bool = True, plot: bool = False):
    ...
    use_weights     whether to use WEIGHT column (e.g. containing e.g. transcript concentrations)
    VIR_max         number of very important references per metric saved in the results 
    r2t             normalize references results relative to target results
    ...
    if use_weights and r2t:
        if ('WEIGHT' in reference_dataset) and ('WEIGHT' in target):
            pass;
        else: raise TypeError('use_weights is set to True, but WEIGHT column is missing in reference_dataset and/or target')
...
def df_map(df, columns = ["identical match", "1nt mismatch", "2nt mismatch", "boltzmann factor"]):
    ...
    for col in columns:
        ...
        plt.xlabel("target site candidates (=queries)")
        yl = plt.yticks()[0]
        yl[yl<0] /= sy
...
def _col_weights(col):
    '''heuristic weights determining how important each binding metric is'''
    col_name = col.name
...
def df_genRanks(DF):  
    '''calculate some overall ranks for sorting'''
    ...
    weighted_ranks = 2**(rel_ranks.apply(stats.norm.ppf)) * DFnum.apply(_col_weights)  # identical matches and boltzman factor are more important than 2nt mismatches. weighted results more important than unweighted -> this function contains heuristcal weight for that
...
### Selection stage 1 ### - select number of samples by their overall score, spatially distributed
def df_select_per_bin(DF, n_best, n_bins = 4, reset = True):  
    '''selects the best (lowest affinity to reference dataset) queries.
        n_best:  how many to select from each bin
        n_bins:  number of position bins (to get results for every region of the target, the results can be divided into bins by their position on the target, then the best are selected in each of these bins )
        reset:   reset previous selection
        returns: total number of selected = n_best * n_bins'''

    df = DF.copy()
    if reset: DF["selected"] = 0
    df['bin'] = pd.cut(df.index, bins=n_bins)
    chosen = DF.index.isin(df.groupby('bin', observed = True )['rank score'].nsmallest(n_best).reset_index(level=0, drop=True).index)
    DF.loc[chosen, "selected"] += 1
    return DF


### Selection stage 2 ### - drop rows with outler values in any column
def df_select_against_outliers(DF, min_select, reset = False): 
    '''drops results where single metrics are worse than average'''
    if reset: DF["selected"] = 0
    position_cols = [col for col in DF.columns if col.startswith("POSITION")]
    o = 0
    df_num = DF[DF["selected"]==DF["selected"].max()].copy().select_dtypes('number')
    df_num = df_num[[col for col in df_num.columns if ("sum" in col or "max" in col)]]
    if len(df_num) < min_select:
        warnings.warn("min_select is less than the current selection to choose from. Set reset=True to ignore previous selection")
        return DF
    while True:
        o += 0.001
        sel = (df_num < o*df_num.median() - (1-o)*df_num.min()).sum(axis = 1 )
        chosen = list(sel[(sel == max(sel.max(), 1))].index)
